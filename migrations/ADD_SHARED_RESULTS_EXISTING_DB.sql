-- =====================================================
-- ADD SHARED RESULTS TO EXISTING SUPABASE DATABASE
-- Purpose: Add share feature to existing database
-- Date: 2026-01-16
-- Instructions: Run this directly in Supabase SQL Editor
-- =====================================================
...
DROP TRIGGER IF EXISTS trigger_update_shared_results_timestamp ON shared_results;
...
DROP POLICY IF EXISTS "Public shared results are viewable by anyone" ON shared_results;
DROP POLICY IF EXISTS "Users can view own shared results" ON shared_results;
DROP POLICY IF EXISTS "Users can create own shared results" ON shared_results;
DROP POLICY IF EXISTS "Users can update own shared results" ON shared_results;
DROP POLICY IF EXISTS "Users can delete own shared results" ON shared_results;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'shared_results') THEN
        RAISE NOTICE '✅ shared_results table created successfully';
    ELSE
        RAISE EXCEPTION '❌ Failed to create shared_results table';
    END IF;
END $$;
...
SELECT 
    ...
    COUNT(*) AS row_count,
    pg_size_pretty(pg_total_relation_size('shared_results')) AS total_size
FROM shared_results;
...
-- Next steps:
-- 1. Backend: Add API routes for creating/fetching shared results
-- 2. Frontend: Add Share button and SharedResult page
