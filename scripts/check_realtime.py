    ...
    try:
        ...
        if result.data:
            ...
            if replica_identity == 'f':
                print(f"   âœ… Replica identity is FULL")
            else:
                print(f"   âŒ Replica identity is {replica_identity} (should be 'f')")
                print(f"   Run: ALTER TABLE jobs REPLICA IDENTITY FULL;")
    except Exception as e2:
        print(f"   âŒ Error: {e2}")
...
try:
    ...
    if result.data:
        ...
        if update_result.data:
            print(f"   âœ… Successfully updated job progress")
            print(f"   If realtime is working, frontend should receive this update")
        else:
            print(f"   âŒ Failed to update job")
    else:
        print(f"   âš ï¸  No jobs found in database")
except Exception as e:
    print(f"   âŒ Error: {e}")
