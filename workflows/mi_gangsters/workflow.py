...
class MiGangstersWorkflow(BaseWorkflow):
    ...
    async def step_image_edit(self, input_data: Dict, step_config: Dict) -> Dict[str, Any]:
        ...
        logger.info("Starting MI Gangsters face swap step")
        ...
        # Image A = user's face (identity source), Image B = MI reference image (base scene)
        user_face_url = input_data['image_url']
        reference_image_b = self.config.get('reference_image_b', 'https://res.cloudinary.com/dnagl4r2t/image/upload/v1775980681/mi_final_d6qfmf.png')

        logger.info(f"Using model: {model}, provider: {provider}")
        logger.info(f"Image A (user face/identity): {user_face_url}")
        logger.info(f"Image B (MI reference/base): {reference_image_b}")
        logger.info(f"Prompt: {prompt[:100]}...")

        # IMPORTANT: Order matters for Nano Banana PRO direct call!
        # First URL = base image (scene to edit) = MI reference
        ...
        generation_params = {
            ...
            'input_image_url': reference_image_b,      # FIRST = base scene (MI)
