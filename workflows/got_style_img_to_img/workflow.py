...
class GotStyleImgToImgWorkflow(BaseWorkflow):
    ...
    async def step_upscale(self, input_data: Dict, step_config: Dict) -> Dict[str, Any]:
        ...
        # Handle raw bytes result from Stability AI
        if result.get('is_raw_bytes'):
            ...
            # If image exceeds 10MB, resize dimensions to fit under limit
            ...
            if image_size_mb > max_upload_size_mb:
                ...
                # Calculate scale factor to target ~9.8MB (with safety margin)
                target_size_mb = 9.8
